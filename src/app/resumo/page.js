...
export default function DashboardAnalitico() {
  ...
  if (statusMotor === "sincronizando") {
    ...
    corMotor = "text-rose-400"; bgMotor = "bg-rose-500/10"; ringStatic = "border-rose-500/20"; ringSpin = "border-t-rose-400"; badgeBorder = "border-rose-500/30";
  ...
  return (
    ...
    <div className="flex h-[100dvh] bg-[#09090b] text-zinc-100 overflow-hidden w-full">
      <Sidebar />
        ...
        <main className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-8 pb-24 w-full">
          <div className="max-w-7xl w-full mx-auto space-y-6 md:space-y-8">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
              <div>
                <h1 className="text-xl md:text-2xl font-bold text-zinc-100 tracking-tight">Painel de Performance</h1>
                <p className="text-xs md:text-sm text-zinc-400 mt-1">Visão omnichannel: WooCommerce e Portal B2B.</p>
              </div>
            </div>
            ...
            {loading ? (
              ...
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                  ...
                  <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-2xl p-5 hover:bg-zinc-800/40 transition-colors w-full">
                    <div className="flex justify-between items-start mb-4">
                      <div className="w-10 h-10 rounded-xl bg-emerald-500/10 flex items-center justify-center">
                        <DollarSign size={20} className="text-emerald-400" />
                      </div>
                      <span className="flex items-center gap-1 text-[10px] md:text-xs font-medium text-emerald-400 bg-emerald-400/10 px-2 py-1 rounded-full border border-emerald-500/20">
                        <TrendingUp size={12} /> Total Omnichannel
                      </span>
                    </div>
                    <p className="text-xs md:text-sm font-medium text-zinc-400">Receita Bruta (Mês)</p>
                    <h3 className="text-xl md:text-2xl font-bold text-zinc-100 mt-1 truncate">{formatarMoeda(dados?.receitaTotal)}</h3>
                  </div>

                  {/* CARD 2: WOOCOMMERCE */}
                  <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-2xl p-5 hover:bg-zinc-800/40 transition-colors relative overflow-hidden w-full">
                    <div className="absolute top-0 right-0 w-16 h-16 bg-purple-500/5 blur-2xl rounded-full"></div>
                    <div className="flex justify-between items-start mb-4">
                      <div className="w-10 h-10 rounded-xl bg-purple-500/10 flex items-center justify-center border border-purple-500/20 shadow-inner shadow-purple-500/10">
                        <ShoppingCart size={20} className="text-purple-400" />
                      </div>
                    </div>
                    <p className="text-xs md:text-sm font-medium text-zinc-400">Pedidos no Woo</p>
                    <h3 className="text-xl md:text-2xl font-bold text-zinc-100 mt-1">{dados?.pedidosWoo || 0}</h3>
                  </div>
                  ...
                  <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-2xl p-5 hover:bg-zinc-800/40 transition-colors relative overflow-hidden w-full">
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div className="w-10 h-10 rounded-xl bg-emerald-500/10 flex items-center justify-center border border-emerald-500/20 shadow-inner shadow-emerald-500/10">
                        <Package size={20} className="text-emerald-400" />
                      </div>
                    </div>
                    <p className="text-xs md:text-sm font-medium text-zinc-400">Pedidos Portal B2B</p>
                    <h3 className="text-xl md:text-2xl font-bold text-zinc-100 mt-1">{dados?.pedidosB2B || 0}</h3>
                  </div>

                  {/* CARD 4: CLIENTES ÚNICOS */}
                  <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-2xl p-5 hover:bg-zinc-800/40 transition-colors w-full">
                    <div className="flex justify-between items-start mb-4">
                      <div className="w-10 h-10 rounded-xl bg-blue-500/10 flex items-center justify-center border border-blue-500/20 shadow-inner shadow-blue-500/10">
                        <Users size={20} className="text-blue-400" />
                      </div>
                    </div>
                    <p className="text-xs md:text-sm font-medium text-zinc-400">Clientes Globais</p>
                    <h3 className="text-xl md:text-2xl font-bold text-zinc-100 mt-1">{dados?.novosClientes || 0}</h3>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                  ...
                  <div className="xl:col-span-2 bg-zinc-900/40 border border-zinc-800/60 rounded-2xl p-4 md:p-6 w-full overflow-hidden">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
                      <h2 className="text-sm md:text-base font-semibold text-zinc-100">Visão de Vendas ({mesFormatado})</h2>
                      <div className="flex flex-wrap items-center gap-3 md:gap-4 text-xs font-medium">
                        <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-purple-500 shadow-[0_0_8px_rgba(168,85,247,0.5)]"></div><span className="text-zinc-400">WooCommerce</span></div>
                        <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]"></div><span className="text-zinc-400">Portal B2B</span></div>
                      </div>
                    </div>
                        ...
                        <AreaChart data={dados?.graficoVendas || []} margin={{ top: 10, right: 0, left: -25, bottom: 0 }}>
                          ...
                          <Tooltip 
                            ...
                            formatter={(value, name) => [
                              formatarMoeda(value), 
                              <span className={name === 'vendasWoo' ? 'text-purple-400' : 'text-emerald-400'}>
                          ...
                          <Area 
                            ...
                            activeDot={{ r: 5, fill: "#6ee7b7", stroke: "#09090b", strokeWidth: 2 }} 
                          />
                          ...
                          <Area 
                            ...
                            activeDot={{ r: 5, fill: "#c4b5fd", stroke: "#09090b", strokeWidth: 2 }} 
                          />
                        </AreaChart>
                      </ResponsiveContainer>
                    </div>
                  ...
                  {/* STATUS DO MOTOR */}
                  <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-2xl p-6 flex flex-col justify-between w-full h-full min-h-[250px]">
                    <div className="flex flex-col h-full">
                      <h2 className="text-sm md:text-base font-semibold text-zinc-100 mb-6">Status do Motor</h2>
                      <div className="flex flex-col items-center justify-center flex-1 space-y-5">
                        ...
                        <div className="text-center">
                          <p className="text-zinc-200 font-medium md:text-lg transition-colors">{textoPrincipal}</p>
