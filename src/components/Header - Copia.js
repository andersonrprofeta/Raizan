...
import { AlertTriangle, Bell, ShoppingCart, Tag, ArrowRight, Zap, X, Plus, Minus, Package, FileText, Menu } from "lucide-react";
...
function ModalResumoCarrinho({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-end p-0 sm:p-6 bg-black/60 backdrop-blur-sm transition-all animate-in fade-in overflow-x-hidden" onClick={onClose}>
      <div className="bg-[#0c0c0e] border-t sm:border border-zinc-800/80 rounded-t-3xl sm:rounded-3xl shadow-2xl w-full sm:max-w-md h-[85vh] sm:h-full sm:max-h-[80vh] flex flex-col overflow-hidden animate-in slide-in-from-bottom-full sm:slide-in-from-right-8" onClick={e => e.stopPropagation()}>
        ...
        {itens.length > 0 && (
          <div className="p-5 border-t border-zinc-800/80 bg-zinc-900/80 space-y-4">
            <div className="flex items-center justify-between gap-3">
              <span className="text-zinc-400 text-sm md:text-base">Subtotal Estimado:</span>
              <span className="text-lg md:text-xl font-bold text-emerald-400">{formatarMoeda(subtotal)}</span>
            </div>
              ...
              return (
                <div key={promo.sku} className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-zinc-900/50 p-4 rounded-2xl border border-zinc-800 hover:border-rose-500/50 transition-all group gap-4 min-w-0">
                  <div className="flex-1 w-full min-w-0">
                    ...
                    <p className="font-bold text-zinc-200 leading-snug text-sm sm:text-base break-words">{promo.nome_produto}</p>
                    <p className="text-lg sm:text-xl font-bold text-emerald-400 mt-2">{formatarMoeda(promo.preco_promocional)}</p>
                  </div>
...
export default function Header() {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      try {
        ...
        // 🟢 DESEMPACOTADOR DE NOME BLINDADO!
        // Às vezes o user fica dentro de obj.user, isso garante que sempre pegaremos o certo:
        ...
        if (isLojista) {
          // Busca em todos os lugares onde o nome pode estar escondido (Incluindo o RAZAO do Oracle)
  ...
  if (!userRole) return <header className="sticky top-0 z-30 w-full h-16 md:mt-8 bg-[#09090b]/80 backdrop-blur-md border-b border-zinc-800/60 overflow-x-hidden" style={{ WebkitAppRegion: 'drag' }}></header>;

  return (
    <>
      <header className="sticky top-0 z-30 w-full min-h-16 md:h-16 md:mt-8 bg-[#09090b]/90 backdrop-blur-md border-b border-zinc-800/60 flex items-center justify-between px-3 sm:px-4 md:px-8 lg:pr-40 py-2 md:py-0 gap-2 sm:gap-3 overflow-x-hidden" style={{ WebkitAppRegion: 'drag' }}>
        
        <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1" style={{ WebkitAppRegion: 'no-drag' }}>
          {/* 🟢 BOTÃO HAMBURGUER (Exclusivo para Mobile) */}
          <button onClick={toggleMobileMenu} className="lg:hidden shrink-0 p-2 bg-zinc-900 rounded-lg text-zinc-400 hover:text-white transition-colors border border-zinc-800/60">
            <Menu size={20} />
          </button>

          <div className="flex flex-col justify-center min-w-0">
            <h2 className="text-xs sm:text-sm md:text-lg font-bold text-zinc-100 flex items-center gap-1 md:gap-2 truncate max-w-[150px] sm:max-w-[220px] md:max-w-none">
            ...
            <span className="hidden sm:block text-xs md:text-sm text-zinc-400 font-medium truncate max-w-[260px] md:max-w-none">{dataHora}</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-2 sm:gap-3 md:gap-5 min-w-0 shrink-0" style={{ WebkitAppRegion: 'no-drag' }}>
          ...
          {userRole === "admin" && diasRestantes !== null && diasRestantes <= 15 && diasRestantes > 0 && (
            <div className="hidden md:flex items-center gap-2 bg-orange-500/10 border border-orange-500/20 px-3 py-1.5 rounded-lg mr-2">
          ...
          {userRole === "lojista" && (
            <div className="flex items-center gap-1.5 sm:gap-2 md:gap-4 min-w-0">
              ...
              <button 
                ...
                className="relative shrink-0 p-1.5 md:p-2 text-zinc-400 hover:text-emerald-400 bg-zinc-900/50 hover:bg-zinc-800 rounded-xl border border-zinc-800 transition-all" 
                title="Ver Resumo do Carrinho"
              >
                <ShoppingCart size={18} className="md:w-5 md:h-5" />
                {qtdCarrinho > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 bg-emerald-500 text-black text-[9px] md:text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center border-2 border-[#09090b]">
                    {qtdCarrinho}
                  </span>
                )}
              </button>
          ...
          {userRole === "admin" && (
            <div className="relative group cursor-pointer mr-0 md:mr-2 shrink-0" style={{ WebkitAppRegion: 'no-drag' }}>
              ...
              {/* TOOLTIP GLASSMORPHISM (Aparece no Hover, escondido no Mobile) */}
              <div className="hidden lg:block absolute right-0 top-full mt-3 w-64 invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0 z-50">
                <div className="bg-[#0c0c0e]/95 backdrop-blur-xl border border-zinc-800 rounded-2xl p-4 shadow-2xl ring-1 ring-white/5">
                  ...
                  <div className="space-y-4">
                    {!updateStatus && <p className="text-xs text-zinc-500 text-center py-2">Nenhuma notificação nova.</p>}
                    {lastPedidoId && (
                      <div className="flex items-start gap-3">
                        <div className="p-1.5 bg-emerald-500/10 rounded-lg"><ShoppingCart size={14} className="text-emerald-400" /></div>
                    ...
                    {lastXmlId && (
                      <div className="flex items-start gap-3">
                        <div className="p-1.5 bg-rose-500/10 rounded-lg"><FileText size={14} className="text-rose-400" /></div>
                        <div className="overflow-hidden">
          ...
          <div className="flex items-center gap-2 sm:gap-3 pl-2 sm:pl-3 md:pl-5 border-l border-zinc-800/60 shrink-0">
            <div className="hidden sm:flex flex-col items-end">
              <span className="text-sm md:text-base font-bold text-zinc-200 leading-tight">{userRole === "lojista" ? "Meu Perfil" : "Operador"}</span>
            ...
            <Link href={userRole === "lojista" ? "/b2b-inicio" : "/conta"}>
              <div className={`w-8 h-8 md:w-10 md:h-10 rounded-xl flex items-center justify-center shadow-lg ring-2 ring-zinc-950 hover:scale-105 transition-all cursor-pointer font-bold text-white text-xs md:text-sm ${userRole === "lojista" ? "bg-gradient-to-br from-emerald-600 to-teal-600 hover:ring-emerald-500/50" : "bg-gradient-to-br from-purple-600 to-indigo-600 hover:ring-purple-500/50"}`}>
                {userInitial}
              </div>
            </Link>
          </div>
