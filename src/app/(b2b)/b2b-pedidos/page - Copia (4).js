                ...
                <div className="pt-2">
                  {/* 🟢 SELEÇÃO DINÂMICA DE PRAZOS (VEM DO ERP) */}
                        {metodoPagamento === 'faturado' && (
                          <div className="animate-in fade-in slide-in-from-top-2 mt-4 space-y-3 bg-zinc-900/30 p-4 rounded-xl border border-zinc-800">
                              ...
                              {user?.prazos_liberados && user.prazos_liberados.length > 0 ? (
                                user.prazos_liberados.map((prazo, index) => (
                                  <button
                                    ...
                                    className={`p-4 rounded-xl border text-left transition-all ${prazoBoleto === prazo ? 'border-emerald-500 bg-emerald-500/10 ring-1 ring-emerald-500/50' : 'border-zinc-800 bg-zinc-900/80 hover:border-zinc-700 hover:bg-zinc-800'}`}
                                  >
                                    <div className="flex items-center gap-3">
                                      <div className={`p-2 rounded-lg ${prazoBoleto === prazo ? 'bg-emerald-500/20 text-emerald-400' : 'bg-zinc-800 text-zinc-400'}`}>
                                ...
                                <div className="col-span-2 p-4 text-center border border-dashed border-zinc-800 rounded-xl bg-zinc-900/50">
                  ...
                  {metodoPagamento === 'pix' && (
                    <div className="animate-in fade-in slide-in-from-top-2 p-3 bg-teal-500/10 border border-teal-500/20 rounded-lg text-teal-400 text-sm flex items-start gap-2">
                      ...
                      <p>O QR Code do Mercado Pago será gerado na próxima tela após a confirmação do pedido.</p>
                            ...
                            {/* 🟢 O FORMULÁRIO DE CARTÃO DO MERCADO PAGO BLINDADO (MODO DARK) */}
                            {metodoPagamento === 'cartao' && (
                              <div className="animate-in fade-in slide-in-from-top-2 mt-4"> {/* 🔴 Removido o bg-white e as bordas brancas! */}
                                
                                {mpKeyMissing ? (
                                  <div className="flex flex-col items-center justify-center p-6 text-center bg-zinc-900/50 rounded-xl border border-zinc-800">
                                    <AlertCircle size={32} className="text-rose-500 mb-3" />
                                    <p className="text-zinc-100 font-bold text-lg">Chave Pública Ausente</p>
                                    <p className="text-zinc-400 text-sm mt-1">Vá no painel de Configurações e adicione a <b>Public Key</b> do Mercado Pago.</p>
                                  </div>
                                ) : subtotal < 2 ? (
                                  <div className="flex flex-col items-center justify-center p-6 text-center bg-zinc-900/50 rounded-xl border border-zinc-800">
                                    <AlertCircle size={32} className="text-amber-500 mb-3" />
                                    <p className="text-zinc-100 font-bold text-lg">Valor Mínimo Não Atingido</p>
                                    <p className="text-zinc-400 text-sm mt-1">As operadoras exigem um pedido mínimo de <b>R$ 2,00</b>.</p>
                                  </div>
                                ) : !isMpReady ? (
                                  <div className="flex flex-col items-center justify-center p-8 text-zinc-500">
                                    ...
                                    <p className="font-medium text-sm text-zinc-400">Conectando ao cofre seguro...</p>
                                  </div>
                                ) : (
                                  <Payment
                                    initialization={{ amount: subtotal }}
                                    customization={{ 
                                      visual: { style: { theme: 'dark' } }, // 🟢 A MÁGICA DO TEMA ESCURO AQUI!
                                    ...
                                    onSubmit={async (param) => {
                                      ...
                                      const resultado = await onFinalizarPedido({ 
                                        ...
                                        prazoBoleto: null, 
                                        metodoEnvio,
                                        dadosCartaoMp: param.formData // Manda o token seguro do cartão pro Motor
                                      });
