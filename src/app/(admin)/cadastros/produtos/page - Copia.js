      ...
      <div className="flex-1 flex flex-col h-screen relative min-w-0">
        ...
        <main className="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-6 lg:p-8">
          <div className="max-w-[1400px] mx-auto space-y-6">
            ...
            {/* TABELA E PAGINAÇÃO ENCAPSULADA */}
            <div className="border border-zinc-200 dark:border-zinc-800/60 bg-white dark:bg-[#0c0c0e] rounded-2xl shadow-md dark:shadow-2xl relative transition-colors flex flex-col">
              ...
              {loading ? (
                ...
                <div className="min-h-[400px] flex flex-col items-center justify-center p-12 text-center animate-in fade-in zoom-in-95 duration-500">
                ...
                <>
                  {/* 🟢 Z-INDEX E OVERFLOW AJUSTADOS AQUI */}
                  <div className="w-full overflow-visible relative z-20 min-h-[350px]"> 
                    <table className="w-full min-w-[1000px] text-sm text-left">
                      ...
                      <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/40 transition-colors">
                        ...
                        {/* 🟢 MAP COM O INDEX PARA A LÓGICA DO MENU */}
                        {produtosPaginados.map((produto, index) => {
                          ...
                          // 🧠 Lógica para o menu abrir para cima se for um dos últimos itens da tela
                          ...
                          if (produto.tipo_produto === 'kit' && produto.composicao_kit) {
                            try {
                              const itens = typeof produto.composicao_kit === 'string' ? JSON.parse(produto.composicao_kit) : produto.composicao_kit;
                              if (Array.isArray(itens) && itens.length > 0) {
                                const somaKit = itens.reduce((acc, curr) => acc + (Number(curr.quantidade || 0) * Number(curr.preco_unitario || 0)), 0);
                                if (!valorExibicao || Number(valorExibicao) === 0) {
                                  valorExibicao = somaKit;
                                }
                              }
                            } catch(e) { console.error("Erro ao ler kit", e) }
                          }
                            ...
                            >
                              ...
                              <td className="px-5 py-3 text-right relative">
                                <div className="flex items-center justify-end gap-2">
                                  ...
                                  {/* 🟢 O MENU AGORA DECIDE SE ABRE PRA BAIXO OU PRA CIMA */}
                                  {isMenuOpen && (
                                    <div ref={menuRef} className={`absolute right-8 ${menuParaCima ? 'bottom-12 mb-1' : 'top-14 mt-1'} w-64 bg-white dark:bg-[#121214] border border-zinc-200 dark:border-zinc-800/80 rounded-xl shadow-2xl dark:shadow-[0_8px_30px_rgba(0,0,0,0.5)] z-50 py-2 text-left animate-in fade-in ${menuParaCima ? 'slide-in-from-bottom-2' : 'zoom-in-95'} duration-200 backdrop-blur-xl`}>
                  ...
                  {/* 🟢 Z-INDEX AJUSTADO PARA A BARRA INFERIOR */}
                  {totalPaginas > 1 && (
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-4 border-t border-zinc-200 dark:border-zinc-800/60 bg-zinc-50/50 dark:bg-zinc-900/20 rounded-b-2xl transition-colors mt-auto relative z-10">
                      <span className="text-sm text-zinc-500 dark:text-zinc-400 font-medium text-center sm:text-left">
                        Mostrando <span className="text-zinc-900 dark:text-zinc-100 font-bold">{indicePrimeiroItem + 1}</span> a <span className="text-zinc-900 dark:text-zinc-100 font-bold">{Math.min(indiceUltimoItem, produtosFiltrados.length)}</span> de <span className="text-zinc-900 dark:text-zinc-100 font-bold">{produtosFiltrados.length}</span> produtos
                      </span>
                      ...
                      <div className="flex gap-2">
                        <button 
                          ...
                          className="p-2 rounded-lg border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 disabled:opacity-30 disabled:cursor-not-allowed transition-colors shadow-sm"
                        >
                          <ChevronLeft size={18} />
                        </button>
                        
                        <div className="px-4 py-2 text-sm font-bold text-zinc-700 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-700 rounded-lg flex items-center bg-white dark:bg-[#121214] shadow-sm">
