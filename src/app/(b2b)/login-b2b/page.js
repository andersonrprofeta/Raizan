...
// O getHubUrl é a única coisa que ele sabe ao nascer.
...
export default function LoginB2B() {
  ...
  // ======================================================================
  // 🟢 A MÁGICA ESTILO "TINY ERP": DESCOBRINDO A IDENTIDADE PELA URL
  // ======================================================================
  useEffect(() => {
    const identificarPortal = async () => {
      try {
        // Ex: portal.rafany.com.br
        const host = window.location.hostname; 
        
        // Pergunta pro Hub Central de quem é esse domínio
        ...
        if (data.success) {
          // Bingo! Salvamos o CNPJ e a URL do Motor Local na memória do navegador.
          // O arquivo api.js vai ler essas variáveis pra sempre!
          ...
          // Se for localhost (seu PC), usa um fallback pra você testar
          if (host === 'localhost' || host === '127.0.0.1') {
            ...
            console.error("Portal não reconhecido na Nuvem.");
          }
        }
      } catch (e) {
        console.error("Falha ao comunicar com o Hub Central.");
      }
    };
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // getApiUrl() agora já tem o motor local certo baixado pelo useEffect acima!
      const response = await fetch(`${getApiUrl()}/api/b2b/auth`, {
        method: "POST",
        headers: getHeaders(), // Já envia o Tenant ID certo
        ...
        <div className="w-full md:w-7/12 bg-white p-8 md:p-12 lg:p-16 flex flex-col justify-center relative">
          ...
          <div className="max-w-md w-full mx-auto">
            ...
            <form onSubmit={handleLogin} className="flex flex-col gap-5">
              ...
              <button
                type="submit"
                disabled={isLoading || !email || senha.length < 6 || !configuracaoPronta}
