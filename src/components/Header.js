...
import { AlertTriangle, Bell, ShoppingCart, Tag, ArrowRight, Zap, X, Plus, Minus, Package } from "lucide-react";
...
function ModalResumoCarrinho({ isOpen, onClose }) {
  ...
  // Cálculo simplificado para o resumo
      ...
      <div className="bg-[#0c0c0e] border border-zinc-800/80 rounded-3xl shadow-2xl w-full max-w-md h-full max-h-[80vh] flex flex-col overflow-hidden animate-in slide-in-from-right-8" onClick={e => e.stopPropagation()}>
        ...
        {itens.length > 0 && (
          <div className="p-5 border-t border-zinc-800/80 bg-zinc-900/80 space-y-4">
            ...
            {/* 🟢 ESSE BOTÃO LEVA PARA A TELA DE CATÁLOGO/CHECKOUT */}
            <Link 
              href="/b2b-pedidos" 
              onClick={onClose}
...
export default function Header() {
  ...
  // 🟢 ESTADO PARA O NOVO MODAL DE CARRINHO
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      setUserRole(user.tipo || "admin");
      isLojista = user.tipo === "lojista";
      
      if (isLojista) {
        setUserName(user.nome || "Cliente B2B");
        setUserEmail(user.cnpj || ""); 
        setUserInitial(gerarIniciais(user.nome));
      }
    } else {
      setUserRole("admin"); 
    }
  ...
  return (
    <>
        ...
        <div className="flex items-center gap-5" style={{ WebkitAppRegion: 'no-drag' }}>
          ...
          {userRole === "lojista" && (
            <div className="flex items-center gap-4 mr-2">
              ...
              {/* 🟢 O ÍCONE DO CARRINHO AGORA ABRE O MODAL */}
              <button 
          ...
          {userRole === "admin" && (
            <div className="relative group cursor-pointer mr-2">
              <Link href={updateStatus ? "/sistema" : "#"} className="transition-colors relative block p-2 bg-zinc-900/50 hover:bg-zinc-800 rounded-xl border border-zinc-800">
                <Bell size={18} className={updateStatus ? "text-purple-400 animate-pulse" : "text-zinc-400 hover:text-zinc-100"} />
                {updateStatus && <span className="absolute top-0 right-0 w-2.5 h-2.5 bg-purple-500 rounded-full border-2 border-[#09090b]"></span>}
              </Link>
            </div>
          )}
      ...
      {/* 🟢 O MODAL DE RESUMO SENDO RENDERIZADO AQUI */}
